...
async def test_mcc_ready_initialization(dut):
    ...
    await mcc_set_regs(dut, {
        0: 0x1E000000,  # MCC_READY + Enable + ClkEn + Div=240
...
async def test_remote_enable_disable(dut):
    ...
    dut.Control0.value = 0x9E000000  # MCC_READY=1, Enable=0, ClkEn=1, Div=240
    await ClockCycles(dut.Clk, 10)
    ...
    dut.Control0.value = 0xDE000000  # MCC_READY=1, Enable=1, ClkEn=1, Div=240
    await ClockCycles(dut.Clk, 100)
...
async def test_iq_phase_relationship(dut):
    ...
    await mcc_set_regs(dut, {
        0: 0xDE010000,  # MCC_READY + Enable + ClkEn + Div=1 (fast)
        1: 0x043C7D00,
        2: 0x64000000
    }, set_mcc_ready=True)
...
async def test_frequency_control(dut):
    ...
    dut.Control0.value = 0xDE0A0000  # Div=10
    await ClockCycles(dut.Clk, 10)
...
async def test_trigger_pulse_generation(dut):
    ...
    await mcc_set_regs(dut, {
        0: 0xDE000000,  # MCC_READY + Enable + ClkEn + Div=0
