...
## Prompt
Dex, you and I have iterated on the variations of the ProbeHero requirements many, many times now.
...
It is tempting to simply utilize the (already synthesized and functioning) VHDL files and testbenches  a 'reference implementation' - but the goal of this task is not to provide VHDL source code. The goal of this task is to distill the requirements and lessons learned through much of the previous iterations into a set of deliverable requirements that, in theory, would allow two different teams to generate functionally equivalent VHDL/Verilog.

Does this approach make sense? We want to preserve the requirements. It may be acceptable to define certain datastructures (such as, a `BasicProbeConfig`) in VHDL (or Verilog). It may also make sense to prescribe a particular filesystem layout / consistency. 


How do you propose we should start. Ask follow-up questions
...
Dex, dont try and generate any new requirements (or code) yet. I would like you to help me think about how to best represent some of the concepts to the requirements. 

Let's start small. Here is a VHDL datastructrure that represents a `BasicProbeConfig`
