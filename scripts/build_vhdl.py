#!/usr/bin/env python3
"""
Modern VHDL build script using GHDL native dependency resolution.

No manual dependency tracking needed! GHDL automatically determines
compilation order by analyzing VHDL 'use' statements.

Usage:
    python scripts/build_vhdl.py              # Build all (import sources)
    python scripts/build_vhdl.py --clean      # Clean build artifacts
    python scripts/build_vhdl.py --entity foo # Build specific entity
    python scripts/build_vhdl.py --help       # Show help

Features:
    - Auto-discovers all VHDL files in modules/
...
def find_vhdl_files(
    category_dirs: List[str] = ["shared", "instruments", "examples", "experimental"]
) -> List[Path]:
    """
    Find all VHDL source files in module directories.
    ...
    Args:
        category_dirs: List of category directories to search
...
def import_all_sources() -> bool:
    ...
    for f in vhdl_files[:5]:
        rel_path = f.relative_to(MODULES_DIR)
        print(f"     - {rel_path}")
    if len(vhdl_files) > 5:
...
def build_all() -> int:
    ...
    print()
    print_status("✅", "Build complete!", Colors.GREEN)
    print("   GHDL has imported all sources and resolved dependencies.")
    print("   To build a specific entity:")
    print(f"   {Colors.BLUE}python scripts/build_vhdl.py --entity <entity_name>{Colors.NC}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Modern VHDL build using GHDL native dependency resolution",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/build_vhdl.py                    # Import all sources
  python scripts/build_vhdl.py --entity foo       # Build entity 'foo'
