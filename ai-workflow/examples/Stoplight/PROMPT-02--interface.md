...
- **@ai-workflow/ng/README-VOLO-STATES.md** - VOLO standard state machine pattern (RESET→READY→IDLE)
...
### VOLO State Pattern Integration:
**All VOLO modules inherit standard state transitions:**
- **RESET_STATE** → **READY_STATE** → **IDLE_STATE** (automatic)
- **Any State** → **FAULT_STATE** (on invalid configuration)
- **Custom states** only after IDLE_STATE (via user trigger)

**Agent Guidelines:**
- **Assume base states exist** - don't ask about RESET/READY/IDLE
- **Focus on custom states** - what happens after IDLE?
- **Use 3-bit state encoding** to allow for custom states
- **Follow transition rules** - no direct custom→base transitions
...
- **VOLO state machine constants** (RESET, READY, IDLE, FAULT + custom states)
- Units constants for all parameters
...
- **VOLO state machine implementation** (RESET→READY→IDLE + custom states)
- Status register implementation
- Simple reset handler with input parameter validation


### 4. Core Testbench (`tb/core/[module_name]_core_tb.vhd`)
- Testbench structure using 4-layer architecture
- **VOLO state machine testing** (RESET→READY→IDLE transitions)
- Test coverage for a simple reset 
- Test that the input configuration parameters are validated
- **Custom state testing** (user trigger → custom states → IDLE)
...
- **VOLO State Machine**: Implement RESET→READY→IDLE pattern + custom states
- **State Machine Encoding**: Use 3-bit std_logic_vector with named constants from package
