...
-- Enhanced version of ProbeHero7 with improved validation and error handling
-- Implements enhanced rules system patterns: SIG-02 (named association) and SIG-03 (signal priority)
...
architecture behavioral of probe_hero8_core is
    ...
    constant STATUS_COOL_BIT    : natural := 3;
    constant STATUS_FIRED_BIT   : natural := 2;
    constant STATUS_FIRING_BIT  : natural := 1;
    constant STATUS_ARMED_BIT   : natural := 0;
    ...
    -- Probe configuration (simplified for this implementation)
...
begin
                ...
                if enable = '1' then
                    ...
                    case current_state is
                        when FIRING_STATE =>
                            ...
                            intensity_out <= to_signed(to_integer(unsigned(cfg_intensity_clamped)) * 16, 16);
                        when others =>
                            trigger_out <= SAFE_TRIGGER_VOLTAGE;
                            intensity_out <= SAFE_INTENSITY_VOLTAGE;
                    end case;
    ...
    -- Configuration validation (simplified for this implementation)
    ...
    cfg_fire_duration_valid <= '1' when cfg_fire_duration_in > 0 and cfg_fire_duration_in <= 65535 else '0';
    
    cfg_cooldown_duration_valid <= '1' when cfg_cooldown_duration_in > 0 and cfg_cooldown_duration_in <= 65535 else '0';
    
    -- Configuration clamping with alarm generation
    ...
    status_update: process(current_state, enable, alarm_fire_duration_clamped, 
                          alarm_cooldown_duration_clamped, alarm_intensity_clamped)
        variable status: std_logic_vector(7 downto 0);
        variable alarm: std_logic;
    begin
        status := (others => '0');
        alarm := alarm_fire_duration_clamped or alarm_cooldown_duration_clamped or alarm_intensity_clamped;
