...
## Trivial Probe Driver (TPD) for Riscure Fault Injection Probes

PLATFORM:  Moku: Go (2 slots, 2 analog inputs, 2 analog outputs)

NOTE: The `TPD` (trivial probe driver) was created as a work-around for the inability to retrieve the control-registers over the network. 

--- 
#JCTODO: Simple diagram here showing the FI-coordinator


---

The scope of the **TBD** has been trimmed even further. In particular, under the design of the __trivial probe driver__ it is assumed that the 
## Trivial-Probe-Driver(TPD): Overview


if everyone got fired a month from now, what do you want to say you spent the last 6 months doing? running a failed investigation or accomplishing <x,y,z> 


Design a controller to drive Riscure fault injection probes (DS1120A EM-FI and DS1110A Laser-FI) from a Moku:Go instrument. The BPD receives trigger signals from a Device Under Test (DUT) and generates control signals to fire the probe with configurable intensity and timing.
